...
Certain emitters may also output [HTML](https://developer.mozilla/en-US/docs/Web/HTML) files. To enable easy customization, these emitters allow you to fully rearrange the layout of the page. The default page layouts can be found in `quartz.layout.ts`.
...
>
> 1. `head` is a single component that renders the `<head>` [tag](https://developer.mozilla/en-US/docs/Web/HTML/Element/head) in the HTML. This doesn't appear visually on the page and is only is responsible for metadata about the document like the tab title, scripts, and styles.
